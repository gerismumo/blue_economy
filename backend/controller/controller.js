...
class DbLearning {
    ...
    async addUsers (requestData) {
        try {
            const query ='INSERT INTO users_list (user_email,user_name, company, phone_number,attendee_interest, attendee_county) VALUES (?,?,?,?,?,?)';
                const values = [
                    requestData.email,
                    requestData.name,
                    requestData.company,
    ...
    async  editUsers(user_id,
        ...
        try {
          const query = `UPDATE users_list SET user_email = ?, user_name = ?, company = ?, phone_number = ?, user_interest = ?, attendee_county= ? WHERE user_id = ? `;
